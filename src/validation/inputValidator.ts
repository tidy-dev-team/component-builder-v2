...
import { ValidationRule, ValidationSchema, figmaValidationRules, validateAgainstSecurity } from './schemas';
import { ValidationError, ValidationResult, ValidationErrorCode, createValidationError, createValidationResult, combineValidationResults } from './validationErrors';
...
export class InputValidator {
   ...
   */
  static validateValue(value: unknown, rule: ValidationRule, fieldName: string): ValidationResult {
   ...
   */
  static validateBuildEventData(data: Record<string, unknown>): ValidationResult {
    const results: ValidationResult[] = [];
   ...
   */
  static validateComponentSetEventData(data: { key?: unknown }): ValidationResult {
    if (!data.key) {
      return createValidationResult(false, [
        createValidationError(
          ValidationErrorCode.REQUIRED_FIELD,
   ...
   */
  static validateDropdownSelection(value: unknown, availableOptions: string[]): ValidationResult {
   ...
   */
  static validateAndSanitizeInput(input: unknown, fieldName: string): { valid: boolean; sanitized: string; errors: ValidationError[] } {
    const errors: ValidationError[] = [];
