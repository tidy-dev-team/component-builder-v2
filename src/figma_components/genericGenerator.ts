 ...
 */
function applyGenericProperties(
  ...
  try {
    ...
    if (variants.length === 0) {
      console.log(`No variants found in component set: ${componentSet.name}`);
      return;
    }
    ...
    console.log(
      `Applied ${usedProperties.length} properties to ${variants.length} variants in ${componentSet.name}`
    );
  } catch (error) {
    errorService.handleError(error, {
 ...
 */
function applyPropertyToVariant(
  ...
  try {
    ...
    if (property.name.includes("#")) {
      ...
      if (variant.variantProperties) {
        ...
        const matchingVariant = findVariantByProperties(
          parentComponentSet,
          currentVariantProps
        );
        ...
        if (matchingVariant) {
          console.log(
            `Applied variant property ${propName}=${variantValue} to ${variant.name}`
          );
        }
      }
    } else {
    ...
    console.log(
      `Failed to apply property ${property.name} to variant ${variant.name}:`,
      error
    );
  }
}
 ...
 */
function applyNonVariantProperty(
  variant: ComponentNode,
  property: ComponentProperty
): void {
  ...
  console.log(
    `Applied property ${property.name}=${property.value} to ${variant.name}`
  );
...
export function getComponentProperties(
  ...
  try {
    ...
    if (firstVariant && firstVariant.variantProperties) {
      Object.entries(firstVariant.variantProperties).forEach(
        ([name, value]) => {
          properties.push({
            name,
    ...
    console.log(
      `Failed to get properties for component set ${componentSet.name}:`,
      error
    );
  }
